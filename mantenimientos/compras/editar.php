...
$stmt = mysqli_prepare($conn, "SELECT * FROM compras WHERE idCompra=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($idProveedor != "" && $fecha != "" && $total != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE compras SET idProveedor=?, fecha=?, total=? WHERE idCompra=?");
        mysqli_stmt_bind_param($stmt2, "isdi", $idProveedor, $fecha, $total, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <select name="idProveedor" required>
      ...
      <option value="<?= $p['idProveedor'] ?>" <?= $p['idProveedor'] == $fila['idProveedor'] ? 'selected' : '' ?>>
    ...
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Compra</button>
  </form>
