...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM usuario WHERE idUsuario=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($usuario != "" && $clave != "" && $rol != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE usuario SET usuario=?, clave=?, rol=? WHERE idUsuario=?");
        mysqli_stmt_bind_param($stmt2, "sssi", $usuario, $clave, $rol, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    <label>Usuario</label>
    <input type="text" name="usuario" value="<?= htmlspecialchars($fila['usuario']) ?>" autocomplete="off" required>
    <label>Contraseña</label>
    <input type="password" name="clave" placeholder="Nueva contraseña" autocomplete="new-password" required>
    <label>Rol</label>
    <select name="rol" required>
      <option value="Administrador" <?= $fila['rol']=="Administrador"?"selected":"" ?>>Administrador</option>
      <option value="Empleado"      <?= $fila['rol']=="Empleado"?"selected":"" ?>>Empleado</option>
      <option value="Supervisor"    <?= $fila['rol']=="Supervisor"?"selected":"" ?>>Supervisor</option>
    </select>
