...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM categoria WHERE idCategoria=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($nombre != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE categoria SET nombre=? WHERE idCategoria=?");
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Categoría</button>
  </form>
