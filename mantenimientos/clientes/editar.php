...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM cliente WHERE idCliente=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($nombre != "" && $apellido != "" && $correo != "" && $dni != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE cliente SET nombre=?, apellido=?, correo=?, dni=? WHERE idCliente=?");
        mysqli_stmt_bind_param($stmt2, "ssssi", $nombre, $apellido, $correo, $dni, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <label>DNI</label>
    <input type="text" name="dni" value="<?= htmlspecialchars($fila['dni']) ?>" maxlength="8" required>
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Cliente</button>
  </form>
