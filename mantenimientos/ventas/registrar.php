...
if(isset($_POST['guardar'])){
    ...
    if($idCliente != "" && $fecha != "" && $total != ""){
        $stmt = mysqli_prepare($conn, "INSERT INTO venta(idCliente,fecha,total) VALUES(?,?,?)");
        mysqli_stmt_bind_param($stmt, "isd", $idCliente, $fecha, $total);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <label>Total (S/.)</label>
    <input type="number" name="total" placeholder="0.00" step="0.01" min="0" required>
    <button type="submit" name="guardar" class="btn-guardar">Guardar Venta</button>
  </form>
