...
$stmt = mysqli_prepare($conn, "SELECT * FROM venta WHERE idVenta=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($idCliente != "" && $fecha != "" && $total != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE venta SET idCliente=?, fecha=?, total=? WHERE idVenta=?");
        mysqli_stmt_bind_param($stmt2, "isdi", $idCliente, $fecha, $total, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Venta</button>
  </form>
