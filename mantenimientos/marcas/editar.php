...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM marca WHERE idMarca=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($nombre != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE marca SET nombre=? WHERE idMarca=?");
        mysqli_stmt_bind_param($stmt2, "si", $nombre, $id);
        mysqli_stmt_execute($stmt2);
        header("Location:listar.php"); exit();
    } else { $error = "Ingrese el nombre"; }
}
?>
<!DOCTYPE html>
<html lang="es">
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Marca</button>
  </form>
