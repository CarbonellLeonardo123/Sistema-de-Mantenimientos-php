...
/** @var mysqli $conn */
$resultado = mysqli_query($conn, "SELECT * FROM reportes ORDER BY fecha DESC");
?>
<!DOCTYPE html>
<html lang="es">
<head>
...
table { width:80%; margin:0 auto; border-collapse:collapse; background:#1a2a3a; border-radius:12px; overflow:hidden; }
...
.badge-rep { padding:4px 14px; border-radius:20px; font-size:13px; font-weight:bold; background:rgba(46,204,113,0.2); color:#2ecc71; }
.fecha { color:#a0bcd8; font-size:13px; }
...
<div class="topbar">
  ...
  <div class="acciones-top">
    ...
    <a href="registrar.php" class="btn-nuevo">+ Nuevo Reporte</a>
  </div>
</div>
<table>
  ...
  <tr>
    ...
    <td><span class="badge-rep"><?= htmlspecialchars($fila['nombre']) ?></span></td>
    <td><span class="fecha">📅 <?= date('d/m/Y', strtotime($fila['fecha'])) ?></span></td>
    <td>
      <a href="editar.php?id=<?= $fila['idReporte'] ?>" class="btn-editar">Editar</a>
      <span class="sep">|</span>
      <a href="eliminar.php?id=<?= $fila['idReporte'] ?>" class="btn-eliminar"
         onclick="return confirm('¿Eliminar este reporte?')">Eliminar</a>
    </td>
  </tr>
  <?php endwhile; ?>
</table>
