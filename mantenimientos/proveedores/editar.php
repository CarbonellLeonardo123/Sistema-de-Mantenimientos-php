...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM proveedor WHERE idProveedor=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($nombre != "" && $telefono != "" && $direccion != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE proveedor SET nombre=?, telefono=?, direccion=? WHERE idProveedor=?");
        mysqli_stmt_bind_param($stmt2, "sssi", $nombre, $telefono, $direccion, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <input type="text" name="direccion" value="<?= htmlspecialchars($fila['direccion']) ?>" required>
