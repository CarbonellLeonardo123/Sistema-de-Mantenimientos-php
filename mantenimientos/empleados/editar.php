...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$stmt = mysqli_prepare($conn, "SELECT * FROM empleado WHERE idEmpleado=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($nombre != "" && $apellido != "" && $cargo != "" && $telefono != "" && $correo != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE empleado SET nombre=?, apellido=?, cargo=?, telefono=?, correo=? WHERE idEmpleado=?");
        mysqli_stmt_bind_param($stmt2, "sssssi", $nombre, $apellido, $cargo, $telefono, $correo, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <label>Cargo</label>
    <input type="text" name="cargo" value="<?= htmlspecialchars($fila['cargo']) ?>" required>
    <label>Teléfono</label>
    <input type="text" name="telefono" value="<?= htmlspecialchars($fila['telefono']) ?>" required>
    <label>Correo</label>
    <input type="email" name="correo" value="<?= htmlspecialchars($fila['correo']) ?>" required>
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Empleado</button>
  </form>
