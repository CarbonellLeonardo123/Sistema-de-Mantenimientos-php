...
/** @var mysqli $conn */
$id = intval($_GET['id']);
$compras   = mysqli_query($conn, "SELECT * FROM compras");
$productos = mysqli_query($conn, "SELECT * FROM producto");
$stmt = mysqli_prepare($conn, "SELECT * FROM detallecompra WHERE idDetalleCompra=?");
...
if(isset($_POST['actualizar'])){
    ...
    if($idCompra != "" && $idProducto != "" && $cantidad != "" && $precio != ""){
        $stmt2 = mysqli_prepare($conn, "UPDATE detallecompra SET idCompra=?, idProducto=?, cantidad=?, precio=?, subtotal=? WHERE idDetalleCompra=?");
        mysqli_stmt_bind_param($stmt2, "iiiddi", $idCompra, $idProducto, $cantidad, $precio, $subtotal, $id);
...
<div class="formulario">
  ...
  <form method="POST">
    ...
    <select name="idCompra" required>
      ...
      <option value="<?= $c['idCompra'] ?>" <?= $c['idCompra'] == $fila['idCompra'] ? 'selected':'' ?>>
        Compra #<?= $c['idCompra'] ?> — <?= date('d/m/Y', strtotime($c['fecha'])) ?>
      </option>
      <?php endwhile; ?>
    </select>
    ...
    <button type="submit" name="actualizar" class="btn-actualizar">Actualizar Detalle</button>
  </form>
